...
#include <../../home/yc/catkin_ws/devel/include/unicore/RawImu.h>
#include <../../home/yc/catkin_ws/src/unicore/include/unicore/unicore_message_extractor.hpp>
...
{       
  ...
  //删除文件目录中的所有文件
	...
	void Write_rawimu(string path,unicore::RawImu ros_raw_imu)
	{
	  ofstream f_rawimu;
	  f_rawimu.open(path,ios::app);
	  RawImu rawimu;
          rawimu.time = ros_raw_imu.header.stamp.toSec();
	 /* rawimu.week=ros_raw_imu.week;
	  rawimu.seconds_into_week=ros_raw_imu.seconds_into_week;
          rawimu.imu_state = ros_raw_imu.imu_state; */
          rawimu.z_accel_output= ros_raw_imu.z_accel_output;
	  rawimu.y_accel_output=ros_raw_imu.y_accel_output;
	  rawimu.x_accel_output= ros_raw_imu.x_accel_output;
	  rawimu.z_gyro_output=ros_raw_imu.z_gyro_output;
	  rawimu.y_gyro_output=ros_raw_imu.y_gyro_output;
	  rawimu.x_gyro_output=ros_raw_imu.x_gyro_output;
	   // f_gnss<<"# rostime,  latitude, longitude, height, north_velocity, east_velocity, up_velocity, roll, pitch, azimuth, status"<<endl;
	 
	 // f_gnss<<"# rostime,  week, second_of_week, latitude, longitude, height, north_velocity, east_velocity, up_velocity, roll, pitch, azimuth, status"<<endl;
	  f_rawimu<<to_string(rawimu.time)<<" "<<to_string(rawimu.z_accel_output)<<" "<<to_string(rawimu.y_accel_output)<<" "<<to_string(rawimu.x_accel_output)<<" "<<to_string(rawimu.z_gyro_output)<<" "<<(rawimu.y_gyro_output)<<" "<<to_string(rawimu.x_gyro_output)<<endl;		  
	  f_rawimu.close();
	}
...
// 	    }
	   ...
	   unicore::RawImu  read_RawImu(string path,double stamp,int l)
	{
	  unicore::RawImu ros_raw_imu;
	   unicore::RawImu rawimu;
	   ifstream f;
	   f.open(path.c_str());
	    char buffer[2000];
	    seek_to_line(f,l);
	  ...
	  while(ros::ok())
	{
	 ...
	 loop_rate.sleep();
	   ...
	   void GetRawImudata(string path,string root_path,unicore::RawImu rosData_rawimu)
	 {
            ros::NodeHandle n;
          n.getParam("/read/root_path", root_path);//加入详细日期 
	  ros::Publisher rawimu_pub = n.advertise<unicore::RawImu>("/rawimu",1);
